...
| [`active/dashboard.md`](active/dashboard.md) | shipped (Phases 0–6 core) | Mobile-first Next.js dashboard — `./maquinista dashboard start/stop/status/logs`, supervised Node child, agent list + detail tabs + KPIs + composer + interrupt/kill/respawn + password auth + audit + rate limit. 42 Go / 40 Vitest / 72 Playwright tests. |
| [`active/dashboard-gaps.md`](active/dashboard-gaps.md) | not started | Living triage of unshipped dashboard surfaces. G.1 global Inbox, G.2 global Conversations, G.3 UI rename (reuses `handle`, id stays stable), G.4 seed default coordinator/planner/coder agents on first boot (idempotent; reconcile loop brings panes back on restart), G.5 spawn new agent from the UI (picks runtime + model from a yaml catalog + soul template, creates the Telegram forum topic and mints `t-<chat>-<thread>` to keep @mention routing matching). Add new gaps to the triage backlog as they surface. |
| [`active/detached-processes.md`](active/detached-processes.md) | not started | Extract a reusable `daemonize` helper, detach both `maquinista start` (renamed to `orchestrator start`) and `dashboard start`, give each symmetric `start / stop / status / logs`, add top-level `maquinista start/stop/status` that run the full stack, and a `maquinista logs` command that tails both streams interleaved under `~/.maquinista/logs/`. |
| [`active/dashboard-rewind-actions.md`](active/dashboard-rewind-actions.md) | not started | Four deferred write actions — rewind, outbox void, outbox reroute, memory pin. Each depends on its respective upstream plan (checkpoint-rollback, a2a, agent-memory). |
| [`active/dashboard-telegram-auth.md`](active/dashboard-telegram-auth.md) | not started | Telegram magic-link auth mode — `/auth` currently renders a stub; this plan wires bot DM → one-time link → session cookie. Depends on the existing dispatcher. |
| [`active/dashboard-cost-sse.md`](active/dashboard-cost-sse.md) | not started | Live KPI + jobs + health updates — add pg_notify triggers to agent_turn_costs / scheduled_jobs / webhook_handlers, subscribe in /api/stream, drop polling refetchInterval in the three client hooks. |
...
| [`active/sandbox-deployment.md`](active/sandbox-deployment.md) | not started | Introduce `sandbox.Provider` abstraction (tmux wraps, E2B managed, Kata/k8s self-hosted) + deploy maquinista to Hetzner k3s (brisakube). Each agent runs in a Firecracker microVM via `kata-fc` RuntimeClass. Covers: sandbox interface, all three backend impls, multi-stage Dockerfiles, GHCR CI, k8s manifests, kata-deploy DaemonSet, Terraform agent node pool, ArgoCD wiring. |
