...
| Gap | Status | Notes |
|---|---|---|
| G.1 Global Inbox | **shipped** (5cde0ca, adjusted) | `listGlobalInbox` + `/api/inbox` + `<GlobalInboxList>`, 3 Playwright specs. Adjusted to show only webhook+scheduler signals (`from_kind NOT IN ('user','agent','system')`); default statuses narrowed to pending+processing+failed+dead. |
